...
/*
    ...
    - 관계형 데이터베이스
    - 데이터들 간에 관게를 ㅣ용해 데이터 중복ㄱ을 최소허ㅏ하는 방식의 DBMS
    ...
    - 레코드 (행, 로우, 튜플) : 한 행에 저장되는 하나의 객체를 나타내는 데이터 묶음
    
    # sql ()
    
    - 관계현 데이터베이스를 관리하기 위한 질문같이 샌긴 명령어
    - 쿼리뮨, 질의문 등으로 불린다
*/

-- tap, tabs, user_tables : 현재 계정이 가지고 있는 테이블들을 볼 수 있는 테이블
SELECT * FROM tab;
SELECT * FROM tabs;
SELECT * FROM user_tables;

--SELECT : 해당 테이블을 조회할 수 있는 질의문
